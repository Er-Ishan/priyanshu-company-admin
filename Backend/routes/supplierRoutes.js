...
function parseAllowedProductIds(value) {
    if (!value) return [];
    if (Array.isArray(value)) {
        return value.map((id) => parseInt(id, 10)).filter((n) => !Number.isNaN(n));
    }
    if (typeof value === "string") {
        try {
            const parsed = JSON.parse(value);
            return Array.isArray(parsed)
                ? parsed.map((id) => parseInt(id, 10)).filter((n) => !Number.isNaN(n))
                : [];
        } catch {
            return [];
        }
...
router.get("/getdata/suppliers", authenticateJWT, async (req, res) => {
    try {
        ...
        let query = `
            SELECT s.*, cs.commission, cs.id as assignment_id, cs.supplier_token, cs.allowed_products
        ...
        for (const row of rows) {
            parseAllowedProductIds(row.allowed_products).forEach((id) => allProductIds.add(id));
        }
        ...
        if (allProductIds.size > 0) {
            ...
            const [products] = await db.promise().query(
                `SELECT id, product_name FROM parking_products WHERE company_id = ? AND id IN (?)`,
                [companyId, ids]
            );
        ...
        const data = rows.map((row) => {
            const allowedIds = parseAllowedProductIds(row.allowed_products);
            return {
                ...row,
                allowed_products: allowedIds,
                allowed_product_names: allowedIds
                    .map((id) => productNameMap[id])
                    .filter(Boolean),
            };
        });
...
// 1b. GET Active parking products for supplier assignment
router.get("/getdata/suppliers/active-products", authenticateJWT, async (req, res) => {
    try {
        const companyId = req.companyId || 1;
        const [rows] = await db.promise().query(
            `SELECT id, product_name, airport_name
...
// 2b. PUT Update Supplier Token (company-scoped assignment)
router.put("/getdata/suppliers/token/:assignmentId", authenticateJWT, async (req, res) => {
    try {
        ...
        const [rows] = await db.promise().query(
            "SELECT id FROM company_suppliers WHERE id = ? AND company_id = ? LIMIT 1",
        ...
        if (!token) {
            token = await generateUniqueSupplierToken(parseInt(assignmentId, 10));
            ...
            if (existing.length > 0) {
                return res.status(400).json({
                    success: false,
                    message: "This supplier token is already in use",
                });
            }
        }

        await db.promise().query(
            "UPDATE company_suppliers SET supplier_token = ? WHERE id = ? AND company_id = ?",
            [token, assignmentId, companyId]
        );

        return res.json({
            success: true,
            message: "Supplier token saved successfully",
            supplier_token: token,
        });
    } catch (err) {
        console.error("PUT Supplier Token Error:", err);
        return res.status(500).json({
            success: false,
            message: err.message || "Failed to update supplier token",
        });
    }
});
...
router.put("/getdata/suppliers/update/:id", authenticateJWT, async (req, res) => {
    try {
        ...
        const {
            ...
            email_parsing_active, supplier_active, allowed_products
        } = req.body;

        let allowedProductsJson = null;
        if (allowed_products !== undefined) {
            const ids = Array.isArray(allowed_products)
                ? allowed_products.map((id) => parseInt(id, 10)).filter((n) => !Number.isNaN(n))
                : [];
            allowedProductsJson = ids.length ? JSON.stringify(ids) : null;
        }

        const updateQuery = `
            UPDATE company_suppliers 
            SET commission = ?, allowed_products = ?
            WHERE supplier_id = ? AND company_id = ?
        `;
        
        const [result] = await db.promise().query(updateQuery, [
            commission, allowedProductsJson, id, companyId
        ]);
  ...
  async (req, res) => {
    try {
      ...
      for (const supplier of suppliers) {
        let whereClause =
          "source = 'Supplier' AND supplier_name = ? AND company_id = ?";
        ...
        const [bookings] = await db.promise().query(
          `SELECT
              ...
              drop_off_date AS dropoff_datetime,
        ...
        const commissionAmount =
          (totalAmount * Number(supplier.commission || 0)) / 100;
