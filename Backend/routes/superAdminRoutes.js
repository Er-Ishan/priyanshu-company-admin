...
import {
    normalizeSlug,
    buildMarketingLink,
    getCompanyBookingBaseUrl,
    ensureMarketingSchema,
    loadMediaSource,
} from "./marketingSourceRoutes.js";
...
async function ensureSupplierTokenColumn() {
    const [cols] = await db.promise().query("SHOW COLUMNS FROM company_suppliers LIKE 'supplier_token'");
    if (cols.length === 0) {
        await db.promise().query(
            "ALTER TABLE company_suppliers ADD COLUMN supplier_token VARCHAR(64) DEFAULT NULL UNIQUE"
        );
    }
}

async function generateUniqueSupplierToken(excludeId = null) {
    for (let attempt = 0; attempt < 10; attempt++) {
        const token = crypto.randomBytes(16).toString('hex');
        const params = excludeId != null ? [token, excludeId] : [token];
        const query = excludeId != null
            ? "SELECT id FROM company_suppliers WHERE supplier_token = ? AND id != ? LIMIT 1"
            : "SELECT id FROM company_suppliers WHERE supplier_token = ? LIMIT 1";
        const [existing] = await db.promise().query(query, params);
        if (existing.length === 0) return token;
    }
    throw new Error("Failed to generate unique supplier token");
}

async function generateUniqueCompanyAccessToken(excludeCompanyId = null) {
    for (let attempt = 0; attempt < 10; attempt++) {
        const token = crypto.randomBytes(16).toString('hex');
        const params = excludeCompanyId != null ? [token, excludeCompanyId] : [token];
        const query = excludeCompanyId != null
            ? "SELECT id FROM companies WHERE access_token = ? AND id != ? LIMIT 1"
            : "SELECT id FROM companies WHERE access_token = ? LIMIT 1";
        const [existing] = await db.promise().query(query, params);
        if (existing.length === 0) return token;
    }
...
async function resolveCompanyAccessToken(accessToken, generateAccessToken, excludeCompanyId = null) {
    let token = typeof accessToken === 'string' ? accessToken.trim() : '';
    if (token) {
        if (!/^[a-f0-9]{32}$/i.test(token)) {
            throw new Error("Access token must be a 32-character hexadecimal string");
        }
        const params = excludeCompanyId != null ? [token, excludeCompanyId] : [token];
        const query = excludeCompanyId != null
            ? "SELECT id FROM companies WHERE access_token = ? AND id != ? LIMIT 1"
        ...
        if (existing.length > 0) {
            throw new Error("This access token is already in use");
        }
        return token;
    }
    if (generateAccessToken) {
        return generateUniqueCompanyAccessToken(excludeCompanyId);
    }
    return null;
}
...
router.patch("/companies/:id", async (req, res) => {
    ...
    const {
        ...
        registration_no, owner_name, access_token
    } = req.body;
    ...
    try {
        ...
        if (access_token !== undefined) {
            const token = access_token === null || access_token === ''
                ? null
                : await resolveCompanyAccessToken(access_token, false, parseInt(id, 10));
        ...
        const message = error.message?.includes("access token") ? error.message : "Failed to update company";
        res.status(error.message?.includes("access token") ? 400 : 500).json({ success: false, message });
    }
});

router.put("/companies/:id/access-token", async (req, res) => {
    const { id } = req.params;
    const { access_token, generate_access_token } = req.body;

    try {
        const [rows] = await db.promise().query("SELECT id FROM companies WHERE id = ?", [id]);
        if (rows.length === 0) {
            return res.status(404).json({ success: false, message: "Company not found" });
        }

        const token = await resolveCompanyAccessToken(
            access_token,
            generate_access_token || !access_token,
            parseInt(id, 10)
        );

        await db.promise().query("UPDATE companies SET access_token = ? WHERE id = ?", [token, id]);
        res.json({ success: true, message: "Access token saved successfully", access_token: token });
    } catch (error) {
        console.error("Update Company Access Token Error:", error);
        res.status(400).json({ success: false, message: error.message || "Failed to update access token" });
    }
});

router.get("/generate-access-token", async (_req, res) => {
    try {
        const access_token = await generateUniqueCompanyAccessToken();
...
router.post("/onboard-company", upload.single('logo'), async (req, res) => {
    const {
        ...
        support_email_address, support_contact_no,
        business_type, business_catrgory, office_hours, ref_prefix,
        registration_no, owner_name, access_token, generate_access_token
    } = req.body;
    ...
    try {
        resolvedAccessToken = await resolveCompanyAccessToken(
            access_token,
            generate_access_token === 'true' || generate_access_token === true
        );
    } catch (err) {
            ...
            const [companyResult] = await connection.query(
                ...
                resolvedAccessToken
                    ? [
                        ...
                        business_type || 'individual', business_catrgory || 'portal',
                        office_hours || null, ref_prefix || 'BKG',
                        registration_no || null, owner_name || null, resolvedAccessToken
                    ]
                    : [
                        name, email, domain || null, mobile_no || null, address || null, logo_url,
                        support_email_address || null, support_contact_no || null,
...
router.post("/company-suppliers/assign", async (req, res) => {
    const { company_id, supplier_id, supplier_from_email, commission, data_from, syncToRegistry, generate_supplier_token } = req.body;
    ...
    try {
        ...
        if (generate_supplier_token) {
            await ensureSupplierTokenColumn();
            supplierToken = await generateUniqueSupplierToken();
        }

        // 1. Perform assignment
        const [result] = await db.promise().query(
            supplierToken
                ? `INSERT INTO company_suppliers (company_id, supplier_id, supplier_from_email, active, data_from, commission, booking_count, supplier_token) 
                   VALUES (?, ?, ?, 1, ?, ?, 0, ?)`
                : `INSERT INTO company_suppliers (company_id, supplier_id, supplier_from_email, active, data_from, commission, booking_count) 
                   VALUES (?, ?, ?, 1, ?, ?, 0)`,
            supplierToken
                ? [company_id, supplier_id, finalEmailJson, data_from || 'both', commission || 0, supplierToken]
        ...
        res.json({
            success: true,
            message: "Supplier assigned and synced successfully",
...
router.put("/company-suppliers/:id/supplier-token", async (req, res) => {
    const { id } = req.params;
    const { supplier_token } = req.body;

    try {
        await ensureSupplierTokenColumn();

        const [rows] = await db.promise().query("SELECT id FROM company_suppliers WHERE id = ?", [id]);
        if (rows.length === 0) {
            return res.status(404).json({ success: false, message: "Assignment not found" });
        }

        let token = typeof supplier_token === 'string' ? supplier_token.trim() : '';
        if (!token) {
            token = await generateUniqueSupplierToken(parseInt(id, 10));
        } else if (!/^[a-f0-9]{32}$/i.test(token)) {
            return res.status(400).json({ success: false, message: "Token must be a 32-character hexadecimal string" });
        } else {
            ...
            if (existing.length > 0) {
                return res.status(400).json({ success: false, message: "This supplier token is already in use" });
            }
        }

        await db.promise().query("UPDATE company_suppliers SET supplier_token = ? WHERE id = ?", [token, id]);
        res.json({ success: true, message: "Supplier token saved successfully", supplier_token: token });
    } catch (error) {
        console.error("Update Supplier Token Error:", error);
        res.status(500).json({ success: false, message: "Failed to update supplier token" });
    }
});

router.put("/company-suppliers/:id", async (req, res) => {
    const { id } = req.params;
    const { active, commission, data_from, supplier_from_email, headerless } = req.body;

    try {
        // Auto-migrate: ensure headerless column exists
        const [cols] = await db.promise().query("SHOW COLUMNS FROM company_suppliers LIKE 'headerless'");
        if (cols.length === 0) {
            await db.promise().query("ALTER TABLE company_suppliers ADD COLUMN headerless TINYINT(1) DEFAULT 0");
        }
        ...
        await db.promise().query(
            `UPDATE company_suppliers 
             SET active = ?, commission = ?, data_from = ?, supplier_from_email = ?, headerless = ?
             WHERE id = ?`,
            [active, commission, data_from, finalEmail, headerless ? 1 : 0, id]
        );
...
router.get("/company-suppliers/:id/mapping", async (req, res) => {
    ...
    try {
        ...
        const [headerlessCols] = await db.promise().query("SHOW COLUMNS FROM company_suppliers LIKE 'headerless'");
        if (headerlessCols.length === 0) {
            await db.promise().query("ALTER TABLE company_suppliers ADD COLUMN headerless TINYINT(1) DEFAULT 0");
        }
        const [colOrderCols] = await db.promise().query("SHOW COLUMNS FROM company_suppliers LIKE 'column_order'");
        if (colOrderCols.length === 0) {
            await db.promise().query("ALTER TABLE company_suppliers ADD COLUMN column_order TEXT DEFAULT NULL");
        }
        // ----------------------------
        ...
        if (!columnToUse) {
            throw new Error("Neither 'email_mapping' nor 'email_parsing' column found.");
        }

        const [csRows] = await db.promise().query(
            `SELECT ${columnToUse} as email_mapping, headerless, column_order FROM company_suppliers WHERE id = ?`, [id]
        );
        ...
        if (csRows[0].column_order) {
            try { columnOrder = JSON.parse(csRows[0].column_order); } catch { columnOrder = null; }
        }
        ...
        res.json({
            ...
            headerless: csRows[0].headerless || 0,
...
router.put("/company-suppliers/:id/column-order", async (req, res) => {
    const { id } = req.params;
    const { column_order } = req.body;

    if (!column_order || typeof column_order !== 'object') {
        return res.status(400).json({ success: false, message: "column_order must be an object mapping index to db_column" });
    }

    try {
        // Auto-migrate
        const [cols] = await db.promise().query("SHOW COLUMNS FROM company_suppliers LIKE 'column_order'");
        if (cols.length === 0) {
            await db.promise().query("ALTER TABLE company_suppliers ADD COLUMN column_order TEXT DEFAULT NULL");
        }

        await db.promise().query(
            "UPDATE company_suppliers SET column_order = ? WHERE id = ?",
            [JSON.stringify(column_order), id]
        );
        res.json({ success: true, message: "Column order saved successfully" });
    } catch (error) {
        console.error("Save Column Order Error:", error);
        res.status(500).json({ success: false, message: "Failed to save column order" });
    }
});
...
router.patch("/bookings/:id/approve", async (req, res) => {
    ...
    try {
        const [result] = await db.promise().query(
            "UPDATE parking_bookings SET status = 'Active' WHERE id = ? AND status = 'pending_review'",
            [id]
        );
        if (result.affectedRows === 0) {
            return res.status(404).json({ success: false, message: "Booking not found or not in pending_review state" });
        }
        res.json({ success: true, message: "Booking approved and set to Active" });
    } catch (error) {
...
router.patch("/bookings/:id/reject", async (req, res) => {
    ...
    try {
        const [result] = await db.promise().query(
            "UPDATE parking_bookings SET status = 'Rejected' WHERE id = ? AND status = 'pending_review'",
            [id]
        );
        if (result.affectedRows === 0) {
            return res.status(404).json({ success: false, message: "Booking not found or not in pending_review state" });
...
// --- MARKETING SOURCES (company-scoped) ---

router.get("/companies/:companyId/marketing-sources", async (req, res) => {
    ...
    try {
        ...
        const data = rows.map((row) => ({
            ...row,
            booking_link: buildMarketingLink(baseUrl, row.acronym || row.slug, row.tracking_code),
            base_url: baseUrl,
        }));
        res.json({
            success: true,
            ...
            company_name: companyRows[0]?.name || null,
        });
    } catch (error) {
        console.error("Super Admin GET marketing-sources error:", error);
        res.status(500).json({
            success: false,
            message: error.sqlMessage || error.message || "Failed to load marketing sources",
        });
    }
});

router.post("/companies/:companyId/marketing-sources", async (req, res) => {
    const companyId = Number(req.params.companyId);
    const { media_source_id, description, is_active } = req.body || {};
    ...
    try {
        ...
        const [result] = await db.promise().query(
            `INSERT INTO marketing_sources (company_id, media_source_id, name, slug, description, is_active)
             VALUES (?, ?, ?, ?, ?, ?)`,
            [
                companyId,
                media.id,
                String(media.media_name).trim(),
                String(media.acronym).trim(),
                description || null,
                is_active === 0 ? 0 : 1,
            ]
        );
        res.status(201).json({ success: true, id: result.insertId, message: "Marketing source created" });
    } catch (error) {
        console.error("Super Admin POST marketing-sources error:", error);
        if (error.code === "ER_DUP_ENTRY") {
            return res.status(409).json({ success: false, message: "This source code already exists" });
        }
        res.status(500).json({ success: false, message: "Failed to create marketing source" });
    }
});

router.put("/companies/:companyId/marketing-sources/:id", async (req, res) => {
    const companyId = Number(req.params.companyId);
    const { media_source_id, description, is_active } = req.body || {};
    ...
    try {
        ...
        const [result] = await db.promise().query(
            `UPDATE marketing_sources
             SET media_source_id = ?, name = ?, slug = ?, description = ?, is_active = ?
        ...
        res.json({ success: true, message: "Marketing source updated" });
    } catch (error) {
        console.error("Super Admin PUT marketing-sources error:", error);
        if (error.code === "ER_DUP_ENTRY") {
            return res.status(409).json({ success: false, message: "This source code already exists" });
        }
        res.status(500).json({ success: false, message: "Failed to update marketing source" });
    }
});

router.delete("/companies/:companyId/marketing-sources/:id", async (req, res) => {
    const companyId = Number(req.params.companyId);
    try {
        const [result] = await db.promise().query(
        ...
        res.json({ success: true, message: "Marketing source deleted" });
    } catch (error) {
        console.error("Super Admin DELETE marketing-sources error:", error);
        res.status(500).json({ success: false, message: "Failed to delete marketing source" });
    }
});

router.get("/companies/:companyId/marketing-sources/analytics", async (req, res) => {
    const companyId = Number(req.params.companyId);
    ...
    try {
        ...
        if (!bookingColumnReady) {
            return res.json({ success: true, from, to, summary: [] });
        }
        ...
        const [summaryRows] = await db.promise().query(summarySql, summaryParams);
        res.json({ success: true, from, to, summary: summaryRows });
    } catch (error) {
        console.error("Super Admin marketing analytics error:", error);
        res.status(500).json({ success: false, message: "Failed to load marketing analytics" });
    }
});
