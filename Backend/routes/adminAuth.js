...
import { resolveMarketingSource } from "./marketingSourceRoutes.js";
...
router.post("/login", async (req, res) => {
  ...
  const query = `
    SELECT u.*, c.name AS company_name, c.domain AS company_domain, c.logo_url AS company_logo, 
...
router.post("/create-admin-booking", async (req, res) => {
  try {
    const body = req.body || {};
    const companyId = req.companyId || 1;
    ...
    const resolvedMarketingSource = await resolveMarketingSource(
      companyId,
      body.marketing_source || body.src
    );

    const valuesPlaceholders = Array(46).fill("?").join(",");
    const sql = `
      INSERT INTO parking_bookings (
        ...
        status, source, marketing_source, website_name, transaction_source, transaction_id,
