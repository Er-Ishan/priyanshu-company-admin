...
router.get("/thomsondata/active", async (req, res) => {
  try {
    const companyId = companyIdFromReq(req);
    const [rows] = await db.promise().query(
      "SELECT * FROM parking_bookings WHERE company_id = ? AND status NOT IN ('Cancelled', 'Pending') ORDER BY created_at DESC LIMIT 2000",
      [companyId]
    );
...
router.get("/thomsondata/daterange", async (req, res) => {
  try {
    ...
    const [rows] = await db.promise().query(
      "SELECT * FROM parking_bookings WHERE company_id = ? AND status NOT IN ('Cancelled', 'Pending') AND (DATE(drop_off_date) >= ? AND DATE(drop_off_date) <= ?) ORDER BY drop_off_date ASC LIMIT 5000",
      [companyId, start, end]
    );
...
router.get("/thomsondata/return-daterange", async (req, res) => {
  try {
    ...
    const [rows] = await db.promise().query(
      "SELECT * FROM parking_bookings WHERE company_id = ? AND status NOT IN ('Cancelled', 'Pending') AND (DATE(return_date) >= ? AND DATE(return_date) <= ?) ORDER BY return_date ASC LIMIT 5000",
      [companyId, start, end]
    );
    return res.json({ success: true, data: rows });
  } catch (error) {
  ...
  try {
    ...
    const [rows] = await db.promise().query(
      `SELECT *,
       ...
       FROM parking_bookings
       WHERE company_id = ? AND status NOT IN ('Cancelled', 'Pending')
         AND (
           (DATE(drop_off_date) >= ? AND DATE(drop_off_date) <= ?)
           OR (DATE(return_date) >= ? AND DATE(return_date) <= ?)
         )
       ORDER BY drop_off_date ASC, return_date ASC
       LIMIT 5000`,
