    ...
    (idErr, idRows) => {
      ...
      const sql = `
        INSERT INTO parking_products (
          id, company_id,
          airport_name,terminals, service_provider, product_name, website_display,
...
router.get("/products", (req, res) => {
  ...
  db.query(sql, [companyId], (err, results) => {
    ...
    const formatted = (results || []).map((p) => {
      if (p.terminals) {
        try {
          p.terminals = JSON.parse(p.terminals).join(", ");
        } catch {
          p.terminals = "";
        }
      }
