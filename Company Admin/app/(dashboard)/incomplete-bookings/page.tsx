...
import { StickyNote, Pencil, Trash2, X, Handshake, SquareParking, MoreVertical, Telescope, Download, Send, FileEdit, Clock, Lightbulb, NotebookPen, Eye } from 'lucide-react';
...
const STATUSES = ['Active', 'No Show', 'Confirmed'] as const;
...
export default function BookingsPage() {
    ...
    const fetchData = async () => {
        ...
        try {
            ...
            const res = await fetch(
                backendProxyPath(`/api/incompletebooking?${qs}`),
                {
    ...
    const sendInvoice = async (bookingId: number) => {
        const res = await fetch(
            backendProxyPath(`/api/admin/send-invoice/${bookingId}`),
            { method: "POST" }
        );
    ...
    const confirmBooking = async (bookingId: number) => {
        const res = await fetch(
            backendProxyPath(`/api/bookings/complete/${bookingId}`),
            {
                method: "POST",
                credentials: "include",
            }
                ...
                <div className="flex flex-col md:flex-row md:items-end justify-between gap-6">
                    <div>
                        ...
                        <p className="text-sm text-muted-foreground mt-3 max-w-lg font-medium">
                            Transactions abandoned during the checkout process. Use this data to identify friction points and perform manual follow-ups.
                        </p>
                    </div>
                        ...
                        <Table>
                            <TableHeader>
                                <TableRow className="hover:bg-transparent border-b border-border/50">
                                    ...
                                    <TableHead className="text-center font-black text-muted-foreground uppercase tracking-widest py-5 px-4 bg-muted/30 text-sm">Status</TableHead>
                                </TableRow>
                            </TableHeader>
                            <TableBody>
                                {loading && (
                                    <TableRow>
                                        <TableCell colSpan={13} className="h-64 text-center">
                                            <div className="flex flex-col items-center justify-center gap-3">
                                                ...
                                                <p className="text-sm font-bold text-muted-foreground animate-pulse">Monitoring traffic...</p>
                                            </div>
                                        </TableCell>
                                    </TableRow>
                                )}
                                    ...
                                    <TableRow>
                                        <TableCell colSpan={13} className="h-64 text-center">
                                            <div className="flex flex-col items-center justify-center gap-2 opacity-40">
                                                ...
                                                <p className="text-lg font-black tracking-tight text-foreground">Cloud clear</p>
                                                <p className="text-xs font-medium text-muted-foreground">No incomplete bookings detected matches your criteria.</p>
                                            </div>
                                        </TableCell>
                                    </TableRow>
                                )}
                                ...
                                {!loading && rows.map((b, i) => (
                                    <Fragment key={b.id}>
                                        <TableRow className="hover:bg-muted/50 transition-colors border-b border-border/50">
                                            ...
                                            <TableCell className="text-center py-4 font-bold text-xs text-muted-foreground uppercase">{b.airport}</TableCell>
                                            ...
                                            <TableCell className="text-center py-4 font-bold text-primary/80">{formatPrettyDateFromAny(b.dropoff_datetime)}</TableCell>
                                            <TableCell className="text-center py-4 font-bold text-primary/80">{formatPrettyDateFromAny(b.return_datetime)}</TableCell>
                                            <TableCell className="text-center py-4 font-black text-xs text-foreground bg-primary/5">{money(b.total_payable)}</TableCell>
                                            <TableCell className="text-center py-4">
                                                <span className={cn(
                                                    "inline-flex px-3 py-1 rounded-full text-[10px] font-black uppercase tracking-widest bg-yellow-500/10 text-yellow-600 border border-yellow-500/20"
                                                )}>
                                                    {b.status || 'Incomplete'}
                                                </span>
                                            </TableCell>
