...
export default function CancellationCharges() {
  ...
  const fetchCharges = async () => {
    setLoading(true);
    try {
      const res = await fetch(backendProxyPath("/api/charges/cancellation"), {
        credentials: "include",
      });
      ...
      console.error("Failed to fetch cancellation charges:", err);
      setRows([]);
    } finally {
      setLoading(false);
    }
  ...
  const addCharge = async () => {
    ...
    if (json?.data) {
      setRows((prev) => [...prev, json.data]); // ✅ APPEND
    }
